...
class Paymentreceipts extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->helper('url');
        $this->load->helper('html');
        $this->load->library("Pdf_paymentreceipt");
    ...
    public function create_pdf_single($code = FALSE) {
        ...
        $pdf = new Pdf_paymentreceipt($PDF_PAGE_ORIENTATION, $PDF_UNIT, $PDF_PAGE_FORMAT, true, 'UTF-8', false);
        ...
        $data1 = $this->payment_model->get_paymentreceipt($code);
        $pdf->AddPage('L', 'A5');
...
//////// set some language-dependent strings (optional)
    ...
    public function view() {
        $page = 'create';
        if (!file_exists(APPPATH . 'views//paymentreceipts//' . $page . '.php')) {
            show_404();
        }
        $this->load->view('header');
        $this->load->view('menu');
        $this->load->view('paymentreceipts/create');
